...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!isLowEnd && !isMobile) {
        ...
        fpsElement.parentElement.style.display = 'none'; // ẩn hiển thị FPS
    }

    ShowToast();
    checkip_address();

    // Typewriter: delay nhỏ để trang load xong trước
    ...
    // Refresh weather mỗi 15 phút (tăng từ 10 phút để giảm request)
    setInterval(() => { if (ipData.ready) updateWeatherData(); }, 900000);
    // Retry IP fallback mỗi 5 phút
    setInterval(() => {
